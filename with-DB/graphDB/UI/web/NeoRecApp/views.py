...
def index(request):
    usage = []
    usage.append(["Get recommendation for user ID: ", "get_reco", RecommendForm])
    usage.append(["Get the list of movies rated by user ID: ", "view_ratings", UserIDForm])
    usage.append(["Update rating for movie ID by user ID: ", "update_rating", UpdateRatingForm])
    usage.append(["List all the movies present in the database: ", "list_movies", ""])
...
def get_recommendations(request):
    ...
    #return HttpResponse(recoID)
    if recoID == "1":
        pci.compute_average_user_rating(userID)
        pci.compute_user_similarity(userID)
        query_result = pci.get_recommendations_collaborative(userID)
    elif recoID == "2":
        pci.compute_average_movie_rating()
        query_result = pci.get_recommendations_content(userID)
    else:
